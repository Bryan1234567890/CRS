...
const MODAL = {
    ...
    track_order_2: () => {
        ...
        if (!txn.first6 || !txn.last4 || txn.first6.length<6 || txn.last4<4 || !txn.firstname || !txn.lastname)
            return;
    ...
    track_now: () => {
        ...
        if (helpOpt == 3 || helpOpt == 4) {
            ...
            if (txn.order_no) {
                ...
                if (!txn.first6 || !txn.last4 || txn.first6.length < 6 || txn.last4 < 4 || !txn.firstname || !txn.lastname || !txn.time || !txn.amount || !txn.email_address)
                    return;
        ...
        if (helpOpt == 5) {
            ...
            if (!txn.order_no || !txn.first6 || !txn.last4 || txn.first6.length < 6 || txn.last4 < 4 || !txn.firstname || !txn.lastname || !txn.email_address)
                return;
    ...
    display_result: (e) => {
        ...
        if (e.responsecode != 200) {
            ...
            shipping_date = shipping_date.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
            $('.ship_date').text(shipping_date);
            $('.courier').text(txn.shipping_carrier);
            $('.tracking_no').text(txn.tracking_number);
            $('.refund-modal-shipping *:not(.track-result-skeleton)').removeClass('hidden');

            $('.product_name').text(txn.order_description);
            let date_settled = txn.action.date.substring(0, 8);
            date_settled = date_settled.substring(0, 4) + '-' + date_settled.substring(4, 6) + '-' + date_settled.substring(6);
            ...
            $('.shipping_name').text(`${txn.shipping_first_name} ${txn.shipping_last_name}`);
    ...
    cancelSsubscription: () => {
        ...
        $.when(TXN.cancelSubscription(params)).done(e => {
            ...
            $('.refund-modal-cancellation-request .inner:not(.skeleton)').removeClass('hidden');
    ...
    reset: () => {
        ...
        $('.refund-modal-shipping *:not(.track-result-skeleton)').addClass('hidden');
        $('.track-result-skeleton').removeClass('hidden');
        $('.opt-button').addClass('hidden');
        $('.refund-modal-next').removeClass('hidden');
        $('.refund-modal-cancellation-request .inner').addClass('hidden');
        $('.refund-modal-cancellation-request .inner.skeleton').removeClass('hidden');
    },
...
$('.refund-modal-txn-user-details input').keyup(function (e) { 
    ...
    if (id == 'order_no') {
        if ($(this).val()) {
            $('.refund-modal-txn-user-details input:not(#order_no)').attr('disabled', true);
            $('.opt-button').addClass('hidden');
            $('.refund-modal-track').removeClass('hidden');
        } else {
            $('.refund-modal-txn-user-details input:not(#order_no)').attr('disabled', false);
            $('.opt-button').addClass('hidden');
            $('.refund-modal-next').removeClass('hidden');
        }
    } else {
        ...
        if (all_empty)
            $('.refund-modal-txn-user-details input#order_no').attr('disabled', false);
        else
...
$('.refund-modal-track-order .refund-modal-txn-user-details input').change(function () {
    let id = $(this).attr('id');
    TXN.user_details[id] = $(this).val();
})

$('.refund-modal-track-membership .refund-modal-txn-user-details input').change(function () {
    let id = $(this).attr('id');
...
$('.refund-modal-txn-user-details input').keypress(function (e) { 
    ...
    if ($(this).val().length >= 6 && id == 'first6')
        e.preventDefault();
    
    if ($(this).val().length >= 4 && id == 'last4')
        e.preventDefault();
});
...
$('.descriptor_key').keyup(debounce(function (e) { 
...
$('.descriptor-searchfield').on('click', ' li:not(.skeleton)', function (e) { 
    ...
    DESCRIPTOR.selected = DESCRIPTOR.results.filter(e => e.descriptor_text == descriptor)[0];
